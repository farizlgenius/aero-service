...
public class CpRepository(AppDbContext context) : ICpRepository
{
    ...
    public async Task<int> CountByMacAndUpdateTimeAsync(int device, DateTime sync)
    {
        ...
        .AsNoTracking()
        .Where(x => x.module.scp_id == device && x.updated_date > sync)
        .CountAsync();
    ...
    public async Task<IEnumerable<ControlPointDto>> GetAsync()
    {
        ...
        .AsNoTracking()
        .OrderBy(x => x.id)
        .Select(x=> new ControlPointDto(x.id,x.cp_id,x.name,x.module_id,x.module.sio_id,x.module_detail,x.output_no,x.relaymode,x.relaymode_detail,x.offlinemode,x.offlinemode_detail,x.default_pulse,x.scp_id,x.location_id,x.is_active))
        .ToArrayAsync();

        return res;
    }

    public async Task<IEnumerable<short>> GetAvailableOpAsync(int ModuleId)
    {
        var ops = await context.module
            .AsNoTracking()
            .Where(sio => sio.id == ModuleId)
    ...
    public async Task<ControlPointDto> GetByIdAsync(int deviceId, int id)
    {
        var dto = await context.control_point
           .Where(x => x.module.scp_id == deviceId && x.id == id)
          .Select(x => new ControlPointDto(x.id, x.cp_id, x.name, x.module_id,x.module.sio_id, x.module_detail, x.output_no, x.relaymode, x.relaymode_detail, x.offlinemode, x.offlinemode_detail, x.default_pulse, x.scp_id, x.location_id, x.is_active))
          .FirstOrDefaultAsync();
    ...
    public async Task<bool> IsAnyByNameAsync(string name)
    {
      ...
      public async Task<bool> IsAnyByDeviceIdAndDriverIdAsync(int deviceid, short driverid)
      {
            return await context.control_point.AsNoTracking().AnyAsync(x => x.scp_id == deviceid && x.cp_id == driverid);
      }
}
