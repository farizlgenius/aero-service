      ...
      public async Task<Aero.Domain.Entities.Transaction> HandleTransactionAsync(IScpReply message)
      {
            ...
            switch (message.tran.source_type)
            {
                  ...
                  case (short)tranSrc.tranSrcScpDiag:
                        switch (message.tran.tran_type)
                        {
                              case (short)tranType.tranTypeSys:
                                    ...
                                    tran.Origin = await context.device.AsNoTracking().OrderBy(x => x.id).Where(x => x.scp_id == (short)message.ScpId).Select(s => s.name).FirstOrDefaultAsync() ?? "";
                  ...
                  case (short)tranSrc.tranSrcSioDiag:
                        tran = await TransactionBuilder(message);
                        tran.Origin = await context.module.AsNoTracking().OrderBy(x => x.id).Where(x => x.scp_id == message.ScpId && x.sio_id == message.tran.source_number).Select(x => x.model_detail).FirstOrDefaultAsync() ?? "";
                        break;
                  ...
                  case (short)tranSrc.tranSrcMP:
                        tran = await TransactionBuilder(message);
                        tran.Origin = await context.monitor_point.AsNoTracking().OrderBy(x => x.id).Where(x => x.scp_id == message.ScpId && x.mp_id == message.tran.source_number).Select(x => x.name).FirstOrDefaultAsync() ?? "";
                  ...
                  case (short)tranSrc.tranSrcCP:
                        tran = await TransactionBuilder(message);
                        tran.Origin = await context.control_point.AsNoTracking().OrderBy(x => x.id).Where(x => x.scp_id == message.ScpId && x.cp_id == message.tran.source_number).Select(x => x.name).FirstOrDefaultAsync() ?? "";
                  ...
                  case (short)tranSrc.tranSrcACR:
                        switch (message.tran.tran_type)
                        {
                              ...
                              case (short)tranType.tranTypeCardBin:
                                    ...
                                    tran.Origin = await context.door.AsNoTracking().OrderBy(x => x.id).Where(x => x.scp_id == message.ScpId && x.acr_id == message.tran.source_number).Select(x => x.name).FirstOrDefaultAsync() ?? "";
                                    break;
    ...
    public async Task<IEnumerable<ModeDto>> GetDeviceAsync(int source)
    {
        switch (source)
        {
            ...
            case (int)tranSrc.tranSrcScpLcl:
                return await context.device.AsNoTracking().Select(x => new ModeDto(x.name,(short)x.scp_id,x.mac)).ToArrayAsync();
            ...
            case (int)tranSrc.tranSrcSioPwr:
                return await context.module.AsNoTracking().Select(x => new ModeDto(x.model_detail,x.sio_id,"")).ToArrayAsync();
            case (int)tranSrc.tranSrcMP:
                return await context.monitor_point.AsNoTracking().Select(x => new ModeDto(x.name,x.mp_id,"")).ToArrayAsync();
            case (int)tranSrc.tranSrcCP:
                return await context.control_point.AsNoTracking().Select(x => new ModeDto(x.name,x.cp_id,"")).ToArrayAsync();
            ...
            case (int)tranSrc.tranSrcAcrRex1:
                return await context.door.AsNoTracking().Select(x => new ModeDto(x.name,x.acr_id,"")).ToArrayAsync();
            case (int)tranSrc.tranSrcTimeZone:
                return await context.timezone.AsNoTracking().Select(x => new ModeDto(x.name,x.timezone_id,"")).ToArrayAsync();
            case (int)tranSrc.tranSrcProcedure:
                return await context.procedure.AsNoTracking().Select(x => new ModeDto(x.name,x.procedure_id,x.procedure_id.ToString())).ToArrayAsync();
            case (int)tranSrc.tranSrcTrigger:
            case (int)tranSrc.tranSrcTrigVar:
                return await context.trigger.AsNoTracking().Select(x => new ModeDto(x.name,x.trigger_id,x.trigger_id.ToString())).ToArrayAsync();
            case (int)tranSrc.tranSrcMPG:
                return await context.monitor_group.AsNoTracking().Select(x => new ModeDto(x.name,x.mpg_id,"")).ToArrayAsync();
            case (int)tranSrc.tranSrcArea:
                return await context.area.AsNoTracking().Select(x => new ModeDto(x.name,(short)x.area_id,"")).ToArrayAsync();
