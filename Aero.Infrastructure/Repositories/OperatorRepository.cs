...
public class OperatorRepository(AppDbContext context) : IOperatorRepository
{
      public async Task<int> AddAsync(Aero.Domain.Entities.Operator data)
      {
        ...
        foreach(var loc in data.LocationIds)
        {
                await context.operator_location.AddAsync(new Aero.Infrastructure.Persistences.Entities.OperatorLocation(loc, en.id));
        }
    ...
    public async Task<IEnumerable<OperatorDto>> GetAsync()
    {
        ...
        var res = data.Select(o => new OperatorDto(o.id, o.username, o.email, o.title, o.first_name, o.middle_name, o.last_name, o.phone, o.image, o.role_id, o.locationd.ToList(), o.is_active)).ToList();
    ...
    public async Task<OperatorDto> GetByIdAsync(int id)
    {
        ...
        return new OperatorDto(o.id,  o.username, o.email, o.title, o.first_name, o.middle_name, o.last_name, o.phone, o.image, o.role_id, o.locationd.ToList(), o.is_active);
    }
    ...
    {
        return await context.@operator.AsNoTracking().Where(x => x.username.Equals(username)).Select(x => x.password).FirstOrDefaultAsync() ?? "";
    }
    ...
    {
        return await context.@operator.AnyAsync(x => x.username.Equals(name));
    }
    ...
    public async Task<bool> IsAnyByNameAsync(string name)
    {
      ...
      public async Task<TokenDataDto> GetTokenDataByUsername(string username)
      {
            ...
            var data =await context.@operator
                .AsNoTracking()
                .Where(x => x.username.Equals(username))
                .Select(x => new TokenDataDto(x.username,x.first_name,x.role_id,x.role.name,x.operator_locations.OrderBy(o => o.location_id).Select(l => l.location_id).FirstOrDefault()))
                .FirstOrDefaultAsync();
      ...
      public async Task<List<int>> GetOperatorLocationsAsync(string username)
      {
            return await context.@operator
                .Where(o => o.username.Equals(username))
                .SelectMany(o => o.operator_locations)
                .Select(l => l.location_id)
                .ToListAsync();
      }

      public async Task<List<PermissionDto>> GetOperatorFeaturesAsync(int operatorId)
      {
           return await context.@operator
                .AsNoTracking()
                .Where(o => o.id == operatorId)
                .SelectMany(o => o.role.permissions)
