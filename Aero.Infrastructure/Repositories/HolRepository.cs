...
public class HolRepository(AppDbContext context) : IHolRepository
{
      ...
      {
            var en = new Aero.Infrastructure.Persistences.Entities.Holiday(data.HolId,data.Name,data.Year,data.Month,data.Day,data.Extend,data.TypeMask,data.LocationId);
    ...
    public async Task<IEnumerable<HolidayDto>> GetAsync()
    {
        var res = await context.holiday
        .AsNoTracking()
        .Select(h => new HolidayDto(h.id,h.hol_id,h.name,h.year,h.month,h.day,h.extend,h.type_mask,h.location_id,h.is_active))
        .ToArrayAsync();
    ...
    public async Task<IEnumerable<HolidayDto>> GetByLocationIdAsync(int locationId)
    {
        ...
        .Select(h => new HolidayDto(h.id, h.hol_id, h.name, h.year, h.month, h.day, h.extend, h.type_mask, h.location_id, h.is_active)).ToArrayAsync();
