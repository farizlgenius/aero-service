...
public class LocationRepository(AppDbContext context) : ILocationRepository
{
    ...
    public async Task<List<string>> CheckRelateReferenceByIdAsync(int id)
    {
        ...
        // company 
        if (await context.location
            .AnyAsync(x => x.id == id && x.companies.Count() > 0)
            )
        {
            errors.Add("Found relate company");
        } 

        // department
        if (await context.location
            .AnyAsync(x => x.id == id && x.departments.Count() > 0)
            )
        {
            errors.Add("Found relate department");
        } 


        // position
         if (await context.location
            .AnyAsync(x => x.id == id && x.positions.Count() > 0)
            )
        {
            errors.Add("Found relate position");
        } 
