...
public class TriggerRepository(AppDbContext context) : ITriggerRepository
{
            ...
            switch (source)
            {
                  ...
                  case (short)tranSrc.tranSrcLoginService:
                        var dtos = await context.device
                            ...
                            .Select(x => new ModeDto(x.name,(short)x.scp_id,x.mac))
                            .ToArrayAsync();
                        return dtos;
                  ...
                  case (short)tranSrc.tranSrcSioPwr:
                        dtos = await context.module
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.model_detail,x.sio_id,""))
                            .ToArrayAsync();
                        return dtos;
                  case (short)tranSrc.tranSrcMP:
                        dtos = await context.monitor_point
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.mp_id,""))
                            .ToArrayAsync();
                        return dtos;
                  case (short)tranSrc.tranSrcCP:
                        dtos = await context.control_point
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.cp_id,""))
                            .ToArrayAsync();
                        return dtos;
                  ...
                  case (short)tranSrc.tranSrcAcrTmprAlt:
                        dtos = await context.door
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.acr_id,""))
                            .ToArrayAsync();
                        return dtos;
                  case (short)tranSrc.tranSrcTimeZone:
                        dtos = await context.timezone
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.timezone_id,""))
                            .ToArrayAsync();
                        return dtos;
                  case (short)tranSrc.tranSrcProcedure:
                        dtos = await context.procedure
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.procedure_id,""))
                            .ToArrayAsync();
                        return dtos;
                  ...
                  case (short)tranSrc.tranSrcTrigVar:
                        dtos = await context.trigger
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.trigger_id,""))
                            .ToArrayAsync();
                        return dtos;
                  case (short)tranSrc.tranSrcMPG:
                        dtos = await context.monitor_group
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.mpg_id,""))
                            .ToArrayAsync();
                        return dtos;
                  case (short)tranSrc.tranSrcArea:
                        dtos = await context.area
                            .AsNoTracking()
                            .Where(x => x.location_id == location)
                            .Select(x => new ModeDto(x.name,x.area_id,""))
                            .ToArrayAsync();
                        return dtos;
                  default:
    ...
    public async Task<IEnumerable<TriggerDto>> GetAsync()
    {
        ...
        .AsNoTracking()
        .OrderBy(x => x.id)
        .Select(en => new TriggerDto(en.id,en.scp_id,en.trigger_id,en.name,en.command,en.procedure_id,en.source_type,en.source_number,en.tran_type,en.code_map.Select(x => new TransactionCodeDto(x.name,x.value,x.description)).ToList()
    ...
    public async Task<IEnumerable<TriggerDto>> GetByLocationIdAsync(int locationId)
    {
        ...
        .OrderBy(x => x.id)
        .Select(en => new TriggerDto(en.id,en.scp_id,en.trigger_id,en.name,en.command,en.procedure_id,en.source_type,en.source_number,en.tran_type,
