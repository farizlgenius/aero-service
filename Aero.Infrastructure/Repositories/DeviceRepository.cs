...
public sealed class DeviceRepository(AppDbContext context) : IDeviceRepository
{
    ...
    public async Task<bool> IsAnyModuleReferenceByDriverIdAsync(int driver)
    {
        ...
        .AsNoTracking()
        .Include(x => x.modules)
        .Where(x => x.scp_id == driver)
        .AnyAsync(x => x.modules.Where(x => x.scp_id != -1).Any());
    }
    ...
    public async Task<IEnumerable<(short DriverId, string Mac)>> GetDriverAndMacAsync()
    {
        var res = await context.device
            .AsNoTracking()
            .Select(x => new { x.scp_id, x.mac })
            .ToArrayAsync();

        return res.Select(x => ((short)x.scp_id, x.mac));
    }

    public async Task<bool> IsAnyByMacAndDriver(string mac, int driver)
    {
        return await context.device.AnyAsync(x => x.mac.Equals(mac) && x.scp_id == driver);
    }
    ...
    public async Task<Pagination<DeviceDto>> GetPaginationAsync(PaginationParamsWithFilter param, int location)
    {
      ...
      {
            return await context.device.AsNoTracking().AnyAsync(x => x.scp_id == Driver);
      }
      ...
      public async Task<string> GetNameByDeviceIdAsync(int device)
      {
            return await context.device.AsNoTracking()
            .OrderBy(x => x.id)
            .Where(x => x.scp_id == device).Select(x => x.name).FirstOrDefaultAsync() ?? "";
      }
}
