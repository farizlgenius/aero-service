...
public class AlvlRepository(AppDbContext context) : IAlvlRepository
{
      public async Task<int> AddAsync(AccessLevel data)
      {
            var en = new Aero.Infrastructure.Persistences.Entities.AccessLevel(
                data.Name,
                data.Components.Select(x => new Aero.Infrastructure.Persistences.Entities.AccessLevelComponent(x.AlvlId,x.DeviceId,x.DoorId,x.AcrId,x.TimezoneId)).ToList(),
                data.LocationId
               );
      ...
      public async Task<int> UpdateAsync(AccessLevel data)
      {
        ...
        var driverids = data.Components.Select(x => x.AlvlId).Distinct().ToList();
    ...
    public async Task<string> GetAcrNameByIdAndDeviceIdAsync(int id, int deviceId)
    {
        ...
        .OrderBy(x => x.id)
        .Where(x => x.id == id && x.scp_id == deviceId)
        .Select(x => x.name)
        .FirstOrDefaultAsync();
    ...
    public async Task<IEnumerable<AccessLevel>> GetDomainAsync()
    {
        ...
        .Select(x => new
        {
            x.id,
            x.name,
            components = x.components.Select(c => new AccessLevelComponent(c.alvl_id, c.scp_id, c.door_id, c.acr_id, c.timezone_id)),
            x.location_id,
            x.is_active
        }).ToArrayAsync();
    ...
    public async Task<IEnumerable<AccessLevelDto>> GetAsync()
    {
        ...
        .Select(x => new
        {
            x.id,
            x.name,
            components = x.components.Select(c => new AccessLevelComponentDto(c.alvl_id, c.scp_id, c.door_id, c.acr_id, c.timezone_id)),
            x.location_id,
            x.is_active
        }).ToArrayAsync();
    ...
    public async Task<IEnumerable<AccessLevelComponent>> GetDoorComponentFromMacAsync(string mac)
    {
        ...
        .AsNoTracking()
        .OrderBy(x => x.id)
        .Where(x => x.components.Any(x => x.scp_id.Equals(mac)))
        .SelectMany(x => x.components.Select(x => new AccessLevelComponent(x.alvl_id,x.scp_id, x.door_id,x.acr_id,x.timezone_id))).ToArrayAsync();
    ...
    public async Task<short> GetLowestUnassignedNumberAsync(int max, int driverid)
    {
        ...
        var query = context.access_level
            .AsNoTracking()
            .Where(x => x.components.Any(x => x.alvl_id == driverid))
