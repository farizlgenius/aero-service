...
public class AreaRepository(AppDbContext context) : IAreaRepository
{
      ...
      {
            var en = new Aero.Infrastructure.Persistences.Entities.AccessArea(data.DeviceId,data.AreaId,data.Name,data.MultiOccupancy,data.AccessControl,data.OccControl,data.OccSet,data.OccMax,data.OccUp,data.OccDown,data.AreaFlag,data.LocationId);
    ...
    public async Task<IEnumerable<AccessAreaDto>> GetAsync()
    {
        var res = await context.area
        .AsNoTracking()
        .Select(a => new AccessAreaDto(a.id,a.scp_id,a.area_id, a.name,a.multi_occ,a.access_control,a.occ_control,a.occ_set,a.occ_max,a.occ_up,a.occ_down,a.area_flag,a.location_id,a.is_active))
        .ToArrayAsync();
