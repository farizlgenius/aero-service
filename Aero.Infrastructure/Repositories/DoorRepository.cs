...
public class DoorRepository(AppDbContext context) : IDoorRepository
{
    ...
    public async Task<IEnumerable<DoorDto>> GetAsync()
    {
        ...
        (
            ...
            d.strike,
            d.sensor,
            d.rexs is null ? new List<RequestExitDto>() : d.rexs.Select(rx => new RequestExitDto((short)rx.module_id, rx.input_no, rx.input_mode, rx.debounce, rx.holdtime, rx.mask_timezone, rx.location_id, rx.is_active)).ToList(),
    ...
    public async Task<DoorDto> GetByIdAsync(int id)
    {
        ...
        (
            ...
            d.access_config,
            d.pair_door_no,
            d.readers.Select(r => new ReaderDto((short)r.module_id, r.reader_no, r.data_format, r.keypad_mode, r.led_drive_mode, r.osdp_flag, r.osdp_baudrate, r.osdp_discover, r.osdp_tracing, r.osdp_address, r.osdp_secure_channel, r.location_id, r.is_active)).ToList(),
            d.reader_out_config,
            d.strike,
            d.sensor,
            d.rexs is null ? new List<RequestExitDto>() : d.rexs.Select(rx => new RequestExitDto((short)rx.module_id, rx.input_no, rx.input_mode, rx.debounce, rx.holdtime, rx.mask_timezone, rx.location_id, rx.is_active)).ToList(),
...
d.default_mode, d.default_mode_detail, d.default_led_mode, d.pre_alarm, d.antipassback_delay, d.strike_t2, d.dc_held2, d.strike_follow_pulse, d.strike_follow_delay,
d.n_ext_feature_type, d.i_lpb_sio, d.i_lpb_number, d.i_lpb_long_press, d.i_lpb_out_sio, d.i_lpb_out_num,
d.df_filter_time, d.is_held_mask, d.is_force_mask,d.access_level_component.Select(),d.location_id, d.is_active
)).ToList();
