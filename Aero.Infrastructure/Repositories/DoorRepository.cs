...
public class DoorRepository(AppDbContext context) : IDoorRepository
{
    ...
    {
        var com = new Persistences.Entities.AccessLevelComponent(2, data.ScpId, data.Id, data.AcrId, 1);
        // Add All Door to Access Level All

        var en = new Persistences.Entities.Door(data.AcrId, data.Name, data.AccessConfig, data.PairDoorNo, data.Direction,data.ScpId, data.LocationId, data.Readers, data.ReaderOutConfiguration,
        data.Strk, data.Sensor, data.RequestExits, data.CardFormat, data.AntiPassbackMode, data.AreaInId,  data.AreaOutId,
         data.SpareTags, data.AccessControlFlags, data.Mode, data.ModeDesc, data.OfflineMode, data.OfflineModeDesc, data.DefaultMode, data.DefaultModeDesc, data.DefaultLEDMode, data.PreAlarm,
    ...
    public async Task<int> ChangeDoorModeAsync(int deviceid, short driverid, short mode)
    {
        ...
        .Where(x => x.scp_id == deviceid && x.acr_id == driverid)
        .OrderBy(x => x.id)
        .FirstOrDefaultAsync();
    ...
    public async Task<int> CountByDeviceIdAndUpdateTimeAsync(int device, DateTime sync)
    {
        ...
        .AsNoTracking()
        .Where(x => x.scp_id == device && x.updated_date > sync)
        .CountAsync();
    ...
    public async Task<IEnumerable<DoorDto>> GetAsync()
    {
        ...
        .Select(d => new
        {
            ...
            d.reader_out_config,
            strike = d.strike == null ? null : new StrikeDto(d.strike.scp_id, (short)d.strike.module_id,d.strike.module.sio_id,d.strike.door_id,d.strike.output_no, d.strike.relay_mode, d.offline_mode, d.strike.strike_max, d.strike.strike_min, d.strike.strike_mode, d.strike.location_id, d.strike.is_active),
            sensor = d.sensor == null ? null : new SensorDto(d.sensor.scp_id,  (short)d.sensor.module_id,d.sensor.module.sio_id,d.sensor.door_id,d.sensor.input_no, d.sensor.input_mode, d.sensor.debounce, d.sensor.holdtime, d.sensor.dc_held, d.sensor.location_id, d.sensor.is_active),
        ...
        (
            ...
            d.card_format, d.antipassback_mode,  d.area_in_id ?? -1,  d.area_out_id ?? -1, d.spare_tag, d.access_control_flag, d.mode, d.mode_detail, d.offline_mode, d.offline_mode_detail,
    ...
    public async Task<IEnumerable<short>> GetAvailableReaderFromModuleIddAsync(int module)
    {
        var reader = await context.module
            .AsNoTracking()
            .Where(cp => cp.id == module)
            .Select(cp => (short)cp.n_reader)
            .FirstOrDefaultAsync();

        var rdrNos = await context.reader
            .AsNoTracking()
            .Where(cp => cp.module_id == module )
    ...
    public async Task<IEnumerable<short>> GetAvailableOsdpAdddressAsync(int module)
    {
        var reader = await context.module
            .AsNoTracking()
            .Where(cp => cp.id == module)
   ...
   d.df_filter_time, d.is_held_mask, d.is_force_mask, d.location_id, d.is_active
    ...
    public async Task<List<short>> GetLowestUnassignedNumberTwoByDeviceIdAsync(int max, int device)
    {
        var first = await GetLowestUnassignedNumberByDeviceIdAsync(max,device);
        if(first == -1) return [-1];
        if(first == 1) return [1,2];
        ...
        {
            if (num != expected)
                return [first,expected]; // found the lowest missing number
            expected++;
        }
    ...
    {
        var dtos = await context.osdp_baudrate.Select(x => new ModeDto(x.name, x.value, x.description)).ToArrayAsync();
    ...
    {
        var dtos = await context.osdp_address.Select(x => new ModeDto(x.name, x.value, x.description)).ToArrayAsync();
...
d.df_filter_time, d.is_held_mask, d.is_force_mask,d.location_id, d.is_active
    ...
    public async Task<bool> IsAnyByNameAsync(string name)
    {
      ...
      public async Task<IEnumerable<ModeDto>> GetDoorTypeAsync()
      {
            var dtos = await context.door_type
                .AsNoTracking()
                .Select(x => new ModeDto(x.name, x.value, x.description))
                .ToArrayAsync();
