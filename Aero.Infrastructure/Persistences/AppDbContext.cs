...
public sealed class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    public DbSet<DoorType> door_type {get; set;}
    public DbSet<Led> led {get; set;}
    public DbSet<LedConfig> led_config {get; set;}
   ...
   .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<Device>()
            .HasMany(p => p.modules)
            .WithOne(p => p.device)
           .HasForeignKey(p => p.scp_id)
            .HasPrincipalKey(p => p.scp_id)
            .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<DeviceCredential>()
            .HasKey(p => new { p.scp_id, p.credential_id });

        modelBuilder.Entity<DeviceCredential>()
            .HasOne(e => e.hardware)
            .WithMany(e => e.device_credentials)
            .HasForeignKey(e => e.scp_id)
            .HasPrincipalKey(e => e.scp_id)
            .OnDelete(DeleteBehavior.Restrict);

        modelBuilder.Entity<UserDevice>()
            .HasKey(p => new { p.scp_id, p.user_id });

        modelBuilder.Entity<UserDevice>()
            .HasOne(x => x.device)
            .WithMany(a => a.user_device)
            .HasForeignKey(a => a.scp_id)
            .HasPrincipalKey(a => a.scp_id)
            .OnDelete(DeleteBehavior.Restrict);
        ...
        modelBuilder.Entity<Aero.Infrastructure.Persistences.Entities.TimeZone>()
        ...
        modelBuilder.Entity<Aero.Infrastructure.Persistences.Entities.TimeZone>()
            ...
            .HasPrincipalKey(x => x.timezone_id)
            .OnDelete(DeleteBehavior.Cascade);



        modelBuilder.Entity<Aero.Infrastructure.Persistences.Entities.TimeZone>().HasData(
            new Aero.Infrastructure.Persistences.Entities.TimeZone { id = 1, name = "Always", timezone_id = 1, mode = 1, active_time = "", deactive_time = "", is_active = true, location_id = 1 }
           );
        ...
        .HasData(
            new DoorType(1,"Single Reader",0,"Single Reader"),
            new DoorType(2,"Dual Reader",1,"Dual Reader"),
            new DoorType(3,"Turnstile Reader",3,"Turnstile Reader")
        );
        ...
        modelBuilder.Entity<OsdpBaudrate>().HasData(
              new OsdpBaudrate { id = 1, value = 1, name = "9600", description = "" },
              new OsdpBaudrate { id = 2, value = 2, name = "19200", description = "" },
              new OsdpBaudrate { id = 3, value = 3, name = "38400", description = "" },
              new OsdpBaudrate { id = 4, value = 4, name = "115200", description = "" },
              new OsdpBaudrate { id = 5, value = 5, name = "57600", description = "" },
              new OsdpBaudrate { id = 6, value = 6, name = "230400", description = "" }
          );

        modelBuilder.Entity<OsdpAddress>().HasData(
            new OsdpAddress { id = 1, value = 0x00, name = "address 0", description = "" },
            new OsdpAddress { id = 2, value = 0x11, name = "address 1", description = "" },
            new OsdpAddress { id = 3, value = 0x22, name = "address 2", description = "" },
            new OsdpAddress { id = 4, value = 0x33, name = "address 3", description = "" }
        );
        ...
        modelBuilder.Entity<AccessArea>()
            .HasMany(a => a.door_in)
            .WithOne(d => d.area_in)
            .HasForeignKey(f => f.area_in_id)
            .HasPrincipalKey(p => p.area_id)
            .IsRequired(false);

        modelBuilder.Entity<AccessArea>()
            .HasMany(a => a.door_out)
            .WithOne(d => d.area_out)
            .HasForeignKey(f => f.area_out_id)
            .HasPrincipalKey(p => p.area_id)
            .IsRequired(false);
        ...
        modelBuilder.Entity<Operator>()
            .HasData(
                new Operator { id = 1, username = "admin", password = "2439iBIqejYGcodz6j0vGvyeI25eOrjMX3QtIhgVyo0M4YYmWbS+NmGwo0LLByUY", email = "support@honorsupplying.com", title = "Mr.", first_name = "Administrator", middle_name = "", last_name = "", phone = "", image = "", role_id = 1 }
            );
        ...
        modelBuilder.Entity<Procedure>()
            ...
            .HasPrincipalKey(x => x.procedure_id);
        ...
        modelBuilder.Entity<Trigger>()
            ...
            .HasPrincipalKey<Procedure>(x => x.procedure_id);

        modelBuilder.Entity<Trigger>()
            .HasMany(x => x.code_map)
            .WithOne(x => x.trigger)
            .HasForeignKey(x => x.value)
            .HasPrincipalKey(x => x.trigger_id)
            .OnDelete(DeleteBehavior.Cascade);
        ...
        .HasMany(x => x.config)
        .WithOne(x => x.led)
        .HasForeignKey(x => x.led_id)
        .OnDelete(DeleteBehavior.Restrict);

        modelBuilder.Entity<LedConfig>()
        .HasData(
            // Table 1
            new LedConfig(1,1,1,0,0,0,0,0,0),
            new LedConfig(2,1,2,2,2,3,3,0,1),
            new LedConfig(3,1,3,3,3,3,3,0,1),
            new LedConfig(4,1,4,1,3,3,3,0,1),
            new LedConfig(5,1,5,1,3,3,3,0,1),
            new LedConfig(6,1,6,1,3,3,3,0,1),
            new LedConfig(7,1,7,1,3,3,3,0,1),
            new LedConfig(8,1,8,1,3,3,3,0,1),
            new LedConfig(9,1,11,3,3,3,3,0,1),
            new LedConfig(10,1,12,2,2,3,3,0,1),
            new LedConfig(11,1,13,2,2,3,3,0,1),
            new LedConfig(12,1,14,0,1,3,3,0,1),
            new LedConfig(13,1,15,0,1,3,3,0,1),
            new LedConfig(14,1,16,0,1,3,3,0,1),

            // Table 2
            new LedConfig(15,2,1,0,0,0,0,0,0),
            new LedConfig(16,2,2,2,2,3,3,0,1),
            new LedConfig(17,2,3,3,3,3,3,0,1),
            new LedConfig(18,2,4,1,3,3,3,0,1),
            new LedConfig(19,2,5,1,3,3,3,0,1),
            new LedConfig(20,2,6,1,3,3,3,0,1),
            new LedConfig(21,2,7,1,3,3,3,0,1),
            new LedConfig(22,2,8,1,3,3,3,0,1),
            new LedConfig(23,2,11,3,3,3,3,0,1),
            new LedConfig(24,2,12,2,2,3,3,0,1),
            new LedConfig(25,2,13,2,2,3,3,0,1),
            new LedConfig(26,2,14,0,1,3,3,0,1),
            new LedConfig(27,2,15,0,1,3,3,0,1),
            new LedConfig(28,2,16,0,1,3,3,0,1),

            // Table 3
            new LedConfig(29,3,1,0,0,0,0,0,0),
            new LedConfig(30,3,2,2,2,3,3,0,1),
            new LedConfig(31,3,3,3,3,3,3,0,1),
            new LedConfig(32,3,4,1,3,3,3,0,1),
            new LedConfig(33,3,5,1,3,3,3,0,1),
            new LedConfig(34,3,6,1,3,3,3,0,1),
            new LedConfig(35,3,7,1,3,3,3,0,1),
            new LedConfig(36,3,8,1,3,3,3,0,1),
            new LedConfig(37,3,11,3,3,3,3,0,1),
            new LedConfig(38,3,12,2,2,3,3,0,1),
            new LedConfig(39,3,13,2,2,3,3,0,1),
            new LedConfig(40,3,14,0,1,3,3,0,1),
            new LedConfig(41,3,15,0,1,3,3,0,1),
            new LedConfig(42,3,16,0,1,3,3,0,1)
        );

        modelBuilder.Entity<Led>()
        .HasData(
            // Table 1
            new Led{ id=1,led_mode=1,location_id=1 },
            new Led{ id=2,led_mode=2,location_id=1 },
            new Led{ id=3,led_mode=3,location_id=1 }
        ...
        .HasData(
             new Source { id = 1, name = "dashboard" },
             new Source { id = 2, name = "events" },
            new Source { id = 3, name = "locations" },
            new Source { id = 4, name = "company" },
            new Source { id = 5, name = "department" },
            new Source { id = 6, name = "position" },
            new Source { id = 7, name = "alerts" },
            new Source { id = 8, name = "operator" },
            new Source { id = 9, name = "role" },
            new Source { id = 10, name = "device" },
            new Source { id = 11, name = "module" },
            new Source { id = 12, name = "point" },
            new Source { id = 13, name = "monitor" },
            new Source { id = 14, name = "monitorgroup" },
            new Source { id = 15, name = "doors" },
            new Source { id = 16, name = "user" },
            new Source { id = 17, name = "level" },
            new Source { id = 18, name = "area"  },
            new Source { id = 19, name = "time" },
            new Source { id = 20, name = "interval" },
            new Source { id = 21, name = "holiday" },
            new Source { id = 22, name = "trigger"},
            new Source { id = 23, name = "procedure" },
            new Source { id = 24, name = "map" },
            new Source { id = 25, name = "report" },
            new Source { id = 26, name = "audit"},
            new Source { id = 27, name = "cardformat" },
            new Source { id = 28, name = "led" },
            new Source { id = 29, name = "credential" },
            new Source { id = 30, name = "diagnostic" },
            new Source { id = 31, name = "settings"},
            new Source { id = 32, name = "license" }
        );
