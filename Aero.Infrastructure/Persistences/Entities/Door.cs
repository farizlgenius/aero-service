...
namespace Aero.Infrastructure.Persistences.Entities
{
    public sealed class Door : BaseEntity,IDeviceId
    {
        ...
        public Door(short driver,string name,int door_type,short pair_door_no,DoorDirection direction,int device_id,int location,List<Aero.Domain.Entities.Reader> readers,short readeroutconfig,Aero.Domain.Entities.Strike k,Aero.Domain.Entities.Sensor s,List<Aero.Domain.Entities.RequestExit> rexs
            ,short cardformat
            ,short antipassbackmode,
            short antipassbackin,
            short antipassbackout,short spare,short acsflag,short mode,string modedetail,short offline,string offlinedetail,short defaultmode,string defaultdetail,
            ...
            ) : base(location)
        {
            ...
            this.scp_id = device_id;
            this.readers = readers.Where(x => x.ScpId > 0).Select(r => 
            new Reader(
                r.ModuleId,
                r.DoorId,
                r.Direction,
            ...
            if(this.strike != null) this.strike = new Strike(k.ScpId,k.ModuleId,k.DoorId,k.OutputNo,k.RelayMode,k.OfflineMode,k.StrkMax,k.StrkMin,k.StrkMode,k.LocationId);
            if(this.sensor != null) this.sensor = new Sensor(s.ScpId,s.ModuleId,s.DoorId,s.InputNo,s.InputMode,s.Debounce,s.HoldTime,s.DcHeld,s.LocationId);
            this.request_exits = rexs.Where(x => x.ScpId > 0).Select(x => new RequestExit(x.ScpId,x.ModuleId,x.DoorId,x.InputNo,x.InputMode,x.Debounce,x.HoldTime,x.MaskTimeZone,x.LocationId)).ToList();
            this.card_format = cardformat;
            this.antipassback_mode = antipassbackmode;
            this.area_in_id = antipassbackin > 0 ? antipassbackin : null;
            this.area_out_id = antipassbackout > 0 ? antipassbackout : null;
        ...
        {
            this.acr_id = data.AcrId;
            this.name = data.Name;
            this.door_type = data.AccessConfig;
            this.pair_door_no = data.PairDoorNo;
            this.direction = data.Direction;
            this.scp_id = data.ScpId;
            this.readers = data.Readers.Select(r => new Reader(r.ModuleId, r.DoorId,r.Direction, r.ReaderNo, r.DataFormat, r.KeypadMode, r.LedDriveMode, r.OsdpFlag, r.OsdpBaudrate, r.OsdpDiscover, r.OsdpTracing, r.OsdpAddress, r.OsdpSecureChannel, r.ScpId, r.LocationId)).ToList();
            this.reader_out_config = data.ReaderOutConfiguration;
            this.strike = new Strike(data.ScpId,data.Strk.ModuleId, data.Strk.DoorId, data.Strk.OutputNo, data.Strk.RelayMode, data.Strk.OfflineMode, data.Strk.StrkMax, data.Strk.StrkMin, data.Strk.StrkMode, data.Strk.LocationId);
            this.sensor = new Sensor(data.ScpId,data.Sensor.ModuleId, data.Sensor.DoorId, data.Sensor.InputNo, data.Sensor.InputMode, data.Sensor.Debounce, data.Sensor.HoldTime, data.Sensor.DcHeld, data.Sensor.LocationId);
            this.request_exits = data.RequestExits.Select(x => new RequestExit(x.ScpId,x.ModuleId, x.DoorId, x.InputNo, x.InputMode, x.Debounce, x.HoldTime, x.MaskTimeZone, x.LocationId)).ToList();
            this.card_format = data.CardFormat;
            this.antipassback_mode = data.AntiPassbackMode;
            this.area_in_id = data.AreaInId > 0 ? data.AreaInId : null;
            this.area_out_id = data.AreaOutId > 0 ? data.AreaOutId : null;
