...
namespace Aero.Application.Services
{
    public sealed class RoleService(IRoleRepository repo) : IRoleService
    {
        public async Task<ResponseDto<RoleDto>> CreateAsync(RoleDto dto)
        {
            ...
            var domain = new Domain.Entities.Role(0,dto.Name,
            dto.Permissions.Select(x => new Permission(x.SourceId,x.Name,
            x.IsAllow,x.IsCreate,x.IsModify,x.IsDelete,x.IsAction))
            .ToList());
        ...
        public async Task<ResponseDto<RoleDto>> UpdateAsync(RoleDto dto)
        {
            ...
            var domain = new Domain.Entities.Role(dto.Id,dto.Name,dto.Permissions.Select(f => 
            new Permission(
                f.SourceId,
                f.Name,
                f.IsAllow,
