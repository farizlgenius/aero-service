    ...
    public sealed class OperatorService(IOperatorRepository repo) : IOperatorService
    {
        ...
        public async Task<ResponseDto<OperatorDto>> GetByUsernameAsync(string Username)
        {
           ...
           var dto = await repo.GetByUsernameAsync(Username);
            ...
            public async Task<List<PermissionDto>> GetOperatorFeaturesAsync(int rolId)
            {
                 return await repo.GetOperatorFeaturesAsync(rolId);
            }

            public async Task<List<int>> GetOperatorLocationsAsync(string username)
            {
                  return await repo.GetOperatorLocationsAsync(username);
            }
