...
namespace Aero.Application.Services
{
    ...
    public class CardFormatService(IDeviceRepository hw,IAeroAdapter cfmt,ICfmtRepository repo,ISettingRepository setting) : ICardFormatService
    {
        ...
        public async Task<ResponseDto<CardFormatDto>> DeleteAsync(int id)
        {
            ...
            var domain = new CardFormat(en.CfmtId,en.Name,en.Fac,en.Offset,en.FuncId,en.Flag,en.Bits,en.PeLn,en.PeLoc,en.PoLn,en.PoLoc,en.FcLn,en.FcLoc,en.ChLn,en.ChLoc,en.IcLn,en.IcLoc);
        ...
        public async Task<ResponseDto<CardFormatDto>> UpdateAsync(CardFormatDto dto)
        {
            ...
            if (en is null) return ResponseHelper.NotFoundBuilder<CardFormatDto>();
            var macs = await hw.GetMacsAsync();
            var domain = new CardFormat(dto.CfmtId,dto.Name,dto.Fac,dto.Offset,dto.FuncId,dto.Flag,dto.Bits,dto.PeLn,dto.PeLoc,dto.PoLn,dto.PoLoc,dto.FcLn,dto.FcLoc,dto.ChLn,dto.ChLoc,dto.IcLn,dto.IcLoc);
            foreach (var mac in macs)
            {
                var id = await hw.GetComponentIdFromMacAsync(mac);
                if (!await  cfmt.CardFormatterConfiguration(id, domain, 1))
