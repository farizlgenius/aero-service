...
namespace Aero.Application.Services{
    public sealed class ProcedureService(IProcedureRepository repo,IDeviceRepository hw,IAeroAdapter proc,ISettingRepository setting) : IProcedureService
    {
        public async Task<ResponseDto<ProcedureDto>> CreateAsync(ProcedureDto dto)
        {
            ...
            var DriverId = await repo.GetLowestUnassignedNumberAsync(ScpSetting.nProc,dto.ScpId);
        ...
        public async Task<ResponseDto<ProcedureDto>> DeleteAsync(int id)
        {
            ...
            if(!proc.ActionSpecificationAsync(en.ProcedureId, [ac]))
            {
                return ResponseHelper.UnsuccessBuilderWithString<ProcedureDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess("", Command.ACTION_SPEC));
            }
