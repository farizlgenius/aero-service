...
{
    public sealed class TriggerService(ITriggerRepository repo, IDeviceRepository hw, IAeroAdapter trig,ISettingRepository setting) : ITriggerService
    {
        public async Task<ResponseDto<TriggerDto>> CreateAsync(TriggerDto dto)
        {
            ...
            dto.TimeZone,dto.TriggerId,dto.LocationId,dto.IsActive);

            if (!trig.TriggerSpecification((short)dto.DeviceId, domain, dto.TriggerId))
            {
