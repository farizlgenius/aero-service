...
{
    public class HolidayService(IDeviceRepository hw,IAeroAdapter aero,IHolRepository repo,ISettingRepository setting) : IHolidayService
    {
        ...
        public async Task<ResponseDto<bool>> ClearAsync()
        {
            ...
            {
                if (!aero.ClearHolidayConfiguration(id))
                {
        ...
        public async Task<ResponseDto<HolidayDto>> CreateAsync(HolidayDto dto)
        {
            ...
            foreach (var id in ids)
            {
                if (!aero.HolidayConfiguration(domain, id))
                {
                   errors.Add(MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync(id), Command.HOL_CONFIG));
        ...
        public async Task<ResponseDto<HolidayDto>> DeleteAsync(int id)
        {
            ...
            var domain = new Holiday(en.Id,en.HolId, en.Name, en.Year, en.Month, en.Day, en.Extend, en.TypeMask, en.LocationId, en.IsActive);
            var ids = await hw.GetDriverIdsAsync();

            foreach (var i in ids)
            {
                if (!aero.DeleteHolidayConfiguration(domain, i))
                {
        ...
        public async Task<ResponseDto<HolidayDto>> UpdateAsync(HolidayDto dto)
        {
            ...
            var domain = new Holiday(dto.Id,dto.HolId,dto.Name,dto.Year,dto.Month,dto.Day,dto.Extend,dto.TypeMask,dto.LocationId,dto.IsActive);
