...
namespace Aero.Application.Services
{
        ...
        ) : IUserService
    {
            ...
            foreach (var alvl in domain.AccessLevels)
            {
                ...
                foreach (var cred in domain.Credentials)
                {
                    ...
                    foreach (var deviceId in deviceIds)
                    {
                        ...
                        if (!user.AccessDatabaseCardRecord((short)deviceId, domain.Flag, cred.CardNo, cred.IssueCode, cred.Pin, alvl.Components.Where(x => x.DeviceId == deviceId).Select(x => x.AlvlId).ToList(), (int)UtilitiesHelper.DateTimeToElapeSecond(cred.ActiveDate), (int)UtilitiesHelper.DateTimeToElapeSecond(cred.DeactiveDate)))
                        {
        ...
        public async Task<ResponseDto<UserDto>> UpdateAsync(UserDto en)
        {
            List<string> errors = new List<string>();

            if (!await repo.IsAnyByUserId(en.UserId)) return ResponseHelper.NotFoundBuilder<UserDto>();
            List<short> CredentialComponentId = new List<short>();

            var dto = await repo.GetByUserIdAsync(en.UserId);
