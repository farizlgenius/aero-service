...
{
    public sealed class MonitorPointService(IMpRepository repo,IAeroAdapter mp,IDeviceRepository hw,ISettingRepository setting) : IMonitorPointService
    {
        ...
        public async Task<ResponseDto<bool>> MaskAsync(MonitorPointDto dto, bool IsMask)
        {
            ...
            if (!mp.MonitorPointMask((short)en.ScpId,dto.MpId, IsMask ? 1 : 0))
            {
                return ResponseHelper.UnsuccessBuilderWithString<bool>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)en.ScpId), Command.SET_MASK));
            }
        ...
        public async Task<ResponseDto<MonitorPointDto>> CreateAsync(MonitorPointDto dto)
        {
            ...
            var DriverId = await repo.GetLowestUnassignedNumberAsync(ScpSetting.nMp,dto.ScpId);
            ...
            if (!mp.InputPointSpecification((short)domain.ScpId, dto.ModuleDriverId, dto.InputNo, dto.InputMode, dto.Debounce, dto.HoldTime))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorPointDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)domain.ScpId), Command.INPUT_SPEC));
            }


            if (!mp.MonitorPointConfiguration((short)domain.ScpId, dto.ModuleDriverId, dto.InputNo, dto.LogFunction, dto.MonitorPointMode, dto.DelayEntry, dto.DelayExit, DriverId))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorPointDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)domain.ScpId), Command.MONITOR_CONFIG));
            }
        ...
        public async Task<ResponseDto<MonitorPointDto>> DeleteAsync(int Id)
        {
            ...
            if (!mp.MonitorPointConfiguration((short)en.ScpId,-1, en.InputNo, en.LogFunction, en.MonitorPointMode, en.DelayEntry, en.DelayExit, en.MpId))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorPointDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)en.ScpId), Command.MONITOR_CONFIG));
            }
        ...
        public async Task<ResponseDto<MonitorPointDto>> UpdateAsync(MonitorPointDto dto)
        {
            ...
            if (!mp.InputPointSpecification((short)domain.ScpId, domain.ModuleDriverId, domain.InputNo, domain.InputMode, domain.Debounce, domain.HoldTime))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorPointDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)domain.ScpId), Command.INPUT_SPEC));
            }

            if (!mp.MonitorPointConfiguration((short)domain.ScpId, domain.ModuleDriverId, domain.InputNo, domain.LogFunction, domain.MonitorPointMode, domain.DelayEntry, domain.DelayExit, domain.MpId))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorPointDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)domain.ScpId), Command.MONITOR_CONFIG));
            }
        ...
        public async Task<ResponseDto<bool>> GetStatusByIdAsync(int id)
        {
            var en = await repo.GetByIdAsync(id);
            if(en is null) return ResponseHelper.NotFoundBuilder<bool>();
            if (!mp.GetMpStatus((short)en.ScpId, en.MpId, 1))
            {
                return ResponseHelper.UnsuccessBuilderWithString<bool>(ResponseMessage.COMMAND_UNSUCCESS,MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)en.ScpId),Command.MP_STATUS));
            }
