...
{
    public sealed class AccessLevelService(IAlvlRepository repo,IDeviceRepository hw,IAeroAdapter aero,ISettingRepository setting) : IAccessLevelService
    {
        ...
        public async Task<ResponseDto<AccessLevelDto>> CreateAsync(CreateAccessLevelDto dto)
        {
            ...
            for(int i = 0;i < ids.Count(); i++)
            {
                ...
                if (!await aero.AccessLevelConfigurationExtended((short)ids.ElementAt(i), DriverId, domain))
                {
        ...
        public async Task<ResponseDto<AccessLevelDto>> DeleteAsync(int id)
        {
            ...
            {
                if (!await aero.AccessLevelConfigurationExtended((short)component.DeviceId,component.AlvlId, 0))
                {
        ...
        public async Task<ResponseDto<AccessLevelDto>> UpdateAsync(AccessLevelDto dto)
        {
            ...
            var domain = new AccessLevel(dto.Id,dto.Name,dto.Components.Select(c => new AccessLevelComponent(c.AlvlId,c.DeviceId,c.DoorId,c.AcrId,c.TimeZoneId)).ToList(),dto.LocationId,dto.IsActive);
            var ids = domain.Components.Select(x => x.DeviceId).Distinct();
            ...
            for (int i = 0; i < ids.Count(); i++)
            {
                ...
                if (!await aero.AccessLevelConfigurationExtended((short)ids.ElementAt(i),domain.Components.Where(x => x.DeviceId == ids.ElementAt(i)).Select(x => x.AlvlId).FirstOrDefault(),domain))
                {
                    errors.Add(MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)ids.ElementAt(i)), Command.ALVL_CONFIG));
