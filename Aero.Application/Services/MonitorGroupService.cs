...
{
    public sealed class MonitorGroupService(IAeroAdapter mpg,IDeviceRepository hw,IMpgRepository repo,ISettingRepository setting) : IMonitorGroupService
    {
        public async Task<ResponseDto<MonitorGroupDto>> CreateAsync(MonitorGroupDto dto)
        {
            ...
            var DriverId = await repo.GetLowestUnassignedNumberAsync(ScpSetting.nMpg,dto.ScpId);
            ...
            if (!mpg.ConfigureMonitorPointGroup((short)dto.ScpId, DriverId, dto.nMpCount, domain.nMpList.ToList()))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorGroupDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)domain.ScpId), Command.CONFIG_MPG));
            }
        ...
        public async Task<ResponseDto<MonitorGroupDto>> DeleteAsync(int id)
        {
            ...
            if (!mpg.ConfigureMonitorPointGroup((short)en.ScpId, en.MpgId, 0, []))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorGroupDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)en.ScpId), Command.CONFIG_MPG));
            }
        ...
        public async Task<ResponseDto<bool>> MonitorGroupCommandAsync(MonitorGroupCommandDto dto)
        {
            ...
            if (!mpg.MonitorPointGroupArmDisarm((short)en.ScpId, en.MpgId, dto.Command, dto.Arg))
            {
                return ResponseHelper.UnsuccessBuilderWithString<bool>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)en.ScpId), Command.MPG_ARM_DISARM));
            }
        ...
        public async Task<ResponseDto<MonitorGroupDto>> UpdateAsync(MonitorGroupDto dto)
        {
            ...
            if (!mpg.ConfigureMonitorPointGroup((short)domain.ScpId, dto.MpgId, dto.nMpCount, domain.nMpList.ToList()))
            {
                return ResponseHelper.UnsuccessBuilderWithString<MonitorGroupDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(await hw.GetMacFromComponentAsync((short)domain.ScpId), Command.CONFIG_MPG));
            }
