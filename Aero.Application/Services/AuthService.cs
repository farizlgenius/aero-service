...
{
    public class AuthService(IOptions<JwtSettings> settings, IOperatorRepository oper,IAuthRepository auth) : IAuthService
    {
        ...
        public async Task<ResponseDto<TokenDtoWithRefresh>> LoginAsync(LoginDto model,string ip)
        {
            ...
            var user = await oper.IsAnyByUsernameAsync(model.Username);
            ...
            await StoreTokenAsync(rawRefresh, data.Username, _refreshTtl,info:ip);
        ...
        public string CreateAccessToken(TokenDataDto data)
        {
            ...
            var claims = new[]
            {
                ...
                new Claim("role_id",data.RoleId.ToString()),
                new Claim("tenant",data.Location.ToString()),
