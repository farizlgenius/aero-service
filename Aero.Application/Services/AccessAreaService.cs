...
{
    public sealed class AccessAreaService(IAreaRepository repo,IDeviceRepository hw,IAeroAdapter aero,ISettingRepository setting) : IAccessAreaService
    {
        ...
        public async Task<ResponseDto<AccessAreaDto>> CreateAsync(CreateAccessAreaDto dto)
        {
            ...
            var domain = new AccessArea(dto.ScpId,DriverId,dto.Name,dto.MultiOccupancy,dto.AccessControl,dto.OccControl,dto.OccSet,dto.OccMax,dto.OccUp,dto.OccDown,dto.AreaFlag,dto.LocationId,dto.Status);
            ...
            foreach(var mac in macs)
            {
               ...
               if (!await aero.ConfigureAccessArea(ScpId, DriverId, domain.MultiOccupancy, domain.AccessControl, domain.OccControl, domain.OccSet, domain.OccMax, domain.OccUp, domain.OccDown, domain.AreaFlag))
               {
        ...
        public async Task<ResponseDto<AccessAreaDto>> UpdateAsync(AccessAreaDto dto)
        {
            ...
            var domain = new AccessArea(dto.ScpId,dto.AreaId, dto.Name, dto.MultiOccupancy,dto.AccessControl,dto.OccControl,dto.OccSet,dto.OccMax,dto.OccUp,dto.OccDown,dto.AreaFlag,dto.LocationId,dto.IsActive);
            ...
            {
                var ScpId = await hw.GetComponentIdFromMacAsync(mac);
                if (!await aero.ConfigureAccessArea(ScpId, dto.AreaId, dto.MultiOccupancy, dto.AccessControl, dto.OccControl, dto.OccSet, dto.OccMax, dto.OccUp, dto.OccDown, dto.AreaFlag))
                {
                    return ResponseHelper.UnsuccessBuilderWithString<AccessAreaDto>(ResponseMessage.COMMAND_UNSUCCESS, MessageBuilder.Unsuccess(mac, Command.AREA_CONFIG));
                }
            }
