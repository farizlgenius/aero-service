...
public class ControlPoint : BaseDomain
{
    public int Id {get; set;}
        public short CpId {get; private set;}
    ...
    public ControlPoint(int Id,short driverid,string name,int moduleid,short moduledriver,string moduledetail,short outputno,short relaymode,string relaymodedetail,short offlinemode,string offlinemodedetail,short defaultpulse,int scpid,int location,bool status) : base(location,status)
    {
        this.Id = Id;
        SetDriverId(driverid);
        SetName(name);
        SetModule(moduleid,moduledetail);
        if(moduledriver < 0) throw new ArgumentException("Module driver id invalid."); 
