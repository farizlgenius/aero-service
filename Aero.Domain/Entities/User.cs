...
public sealed class User : BaseDomain
{
       ...
       public string Identification {get; private set;} = string.Empty;
        ...
        public User(string userid,string Identification,string title,string firstname,string middlename,string lastname,Gender sex,DateTime dateOfBirth,string email,string phone,int company_id,string company,int position_id,string position,int department_id,string department,string address,string image,short flag,List<string> additional,List<Credential> creds,List<AccessLevel> alvl,int locationId,bool status) : base(locationId,status)
        {
            SetUserId(userid);
    ...
    private void SetIdentification(string identitification)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(identitification);
        if(!RegexHelper.IsValidOnlyCharAndDigit(identitification)) throw new ArgumentException("Identification invalid.");
        this.Identification = identitification;
    }
